---
name: strategy-agent
description: Generates the strategy artifact for the AI PM OS pipeline. Invoked by the orchestrator, or directly if the user just wants this artifact on its own.
tools: Read, Write
model: inherit
---
...
Reads `discovery.opportunity_areas` and `vision.north_star_metric`. If that upstream context is missing from
